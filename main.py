...
def process_single_file(xml_file_path: str, json_file_path: str) -> None:
    ...
    try:
        ...
        logging.info(
            "Token dictionary generated successfully at %s", token_output_file_path
        )
        ...
        rdf_xml_output_file_path = os.path.join(
            output_dir, f"{file_name}_named_entities.rdf"
        )
        ...
        g_tokenized: Graph = create_rdf_graph_from_tokenized_word(
            metadata, token_dict, g_metadata
        )
        # Serialize RDF graph to RDF/XML
        rdf_xml_output_file_path = os.path.join(
            output_dir, f"{file_name}_named_entities.rdf"
        ...
        logging.info(
            "RDF graph serialized to RDF/XML for tokenized words at %s",
            rdf_xml_output_file_path,
        )

        # Generate RDFa from the graph
        rdfa_html = generate_rdfa_from_graph(
            g_metadata, os.path.splitext(os.path.basename(xml_file_path))[0]
        )
        ...
        with open(rdfa_html_output_file_path, "wb") as f:
            f.write(rdfa_html)
        logging.info(
            "RDFa XHTML file generated successfully at %s", rdfa_html_output_file_path
        )

        # Save Turtle serialization for the first graph (metadata)
        turtle_metadata_file_path = os.path.join(
            output_dir, f"{file_name}_metadata.ttl"
        )
        save_turtle_serialization(g_metadata, turtle_metadata_file_path)
        logging.info(
            "Turtle serialization file for metadata generated successfully at %s",
            turtle_metadata_file_path,
        )

        # Save Turtle serialization for the second graph (tokenized words)
        turtle_tokenized_file_path = os.path.join(
            output_dir, f"{file_name}_tokenized.ttl"
        )
        save_turtle_serialization(g_tokenized, turtle_tokenized_file_path)
        logging.info(
            "Turtle serialization file for tokenized words generated successfully at %s",
            turtle_tokenized_file_path,
        )
